...
class TrafficSignal:
    ...
    def __init__(
        ...
        # print("tetstststts: ", self.sumo.lanearea.getIDList())
        
        self.lanes_length = {lane: self.sumo.lane.getLength(lane) for lane in self.lanes + self.out_lanes}
        # self.lanes_length = {lane: 50 for lane in self.lanes + self.out_lanes}
        # print("lanes_length: ", self.lanes_length)
    ...
    def get_lanes_density(self) -> List[float]:
        ...
        average_vehs = np.mean([
            self.sumo.lane.getLastStepVehicleNumber(lane)
            for lane in self.lanes
        ])
        return [min(1, density) for density in lanes_density], average_vehs

    def getJamLength(self):
        detIds = self.sumo.lanearea.getIDList()
        jamlengths = np.array([self.sumo.lanearea.getJamLengthMeters(detId) for detId in detIds])
        # Normalize to 0->10
        if np.max(jamlengths) - np.min(jamlengths) == 0:
            return jamlengths
        normalized_jamlengths = (jamlengths - np.min(jamlengths)) / (np.max(jamlengths) - np.min(jamlengths)) * 10
        
        # jamlengths = np.array(jamlengths).reshape(-1, 2)
        # jamlengths = np.mean(jamlengths, axis=1)
        return normalized_jamlengths
        
    def get_number_vehs_in_inbound_intersection(self):
        detIds = self.sumo.lanearea.getIDList()
        nums = np.array([self.sumo.lanearea.getLastStepVehicleNumber(detId) for detId in detIds])
        if np.max(nums) - np.min(nums) == 0:
            return nums
        normalized_nums = (nums - np.min(nums)) / (np.max(nums) - np.min(nums)) * 10
    ...
    def get_avg_speed_in_inbound_intersection(self):
        detIDs = self.sumo.lanearea.getIDList()
        speed = [self.sumo.lanearea.getLastStepMeanSpeed(detId) for detId in detIDs]
        if np.max(speed) - np.min(speed) == 0:
            return speed
        normalized_speed = (speed - np.min(speed)) / (np.max(speed) - np.min(speed)) * 10
        # speed = np.array(speed).reshape(-1, 2)
        # mean_avg_speed = np.mean(speed, axis=1)
